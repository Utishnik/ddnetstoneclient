...
/*я точно не уверен но как будто такая логика короче в m_file карта и ее состояние 
записи,и она одновремнно обновляется + читается я хз надо рабоьрать в путях m file  */
void CDemoPlayer::DoTick()
{
	...
	while(true)
	{
		...
		if(ChunkSize)
		{
			printf("%s\t%d\n","Read Start",ftell(m_File));
			if(io_read(m_File, m_aCompressedSnapshotData, ChunkSize) != (unsigned)ChunkSize) // тут я остановился
			{
				Stop("Error reading chunk data");
				break;
			}
			printf("%s\t%d\n","Read End",ftell(m_File));
